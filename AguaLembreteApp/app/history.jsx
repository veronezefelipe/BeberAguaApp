...
export default function HistoryScreen() {
  ...
  const prepararDadosGrafico = (dados) => {
    const grafico7Dias = dados.slice(0, 7).reverse();
    
    const labels = grafico7Dias.map(item => {
      const [dia, mes] = item.date.split("/");
      return `${dia}/${mes}`;
    });
    
    const valores = grafico7Dias.map(item => item.count);
  ...
  const carregar = async () => {
    try {
      ...
      if (savedHistory) {
        ...
        setDadosGrafico(prepararDadosGrafico(ordenado));
      }
    } catch (e) {
